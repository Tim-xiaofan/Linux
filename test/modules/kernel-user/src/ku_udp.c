 ...
 */
static int 
ku_ip_recv_error(struct sock *sk, struct msghdr *msg, int len)
{
		...
		size_t len, int noblock, int flags, int *addr_len)
{
	...
	if (flags & MSG_ERRQUEUE)
		return ku_ip_recv_error(sk, msg, len);
...
try_again:
	...
	if (skb_csum_unnecessary(skb))
		...
		/** 
		 * int skb_copy_and_csum_datagram_iovec(struct sk_buff *skb, int hlen, struct iovec *iov, int len);
		 * */
...
int ku_udp_sendmsg(struct kiocb *iocb, struct socket *sock, struct msghdr *msg, size_t len)
{
	...
	return udp_sendmsg(iocb, sock->sk, msg, len);
}
